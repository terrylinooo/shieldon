<?php defined('SHIELDON_VIEW') || exit('Life is short, why are you wasting time?');
/*
 * This file is part of the Shieldon package.
 *
 * (c) Terry L. <contact@terryl.in>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

use function Shieldon\Helper\_e;
use function Shieldon\Helper\mask_string;

$timezone = '';

?>

<form method="post">

<div class="so-setting-page">
    <div class="so-tab">
        <?php $this->_include('panel/system_layer_firewall/tab'); ?>
            
        <section id="content1" class="tab-section">
            <?php $this->_include('panel/system_layer_firewall/manager'); ?>
        </section>

        <section id="content2" class="tab-section">
            <?php $this->_include('panel/system_layer_firewall/status'); ?>
        </section>
    </div>
    <div class="d-flex justify-content-center py-2">
        <button type="submit" class="btn btn-enter"><i class="fas fa-fire-alt"></i> <?php _e('panel', 'overview_btn_save', 'SAVE'); ?></button>
    </div>
</div>
<?php $this->_csrf(); ?>
<input type="hidden" name="tab" value="system-firewall-setting">
<input type="hidden" name="managed_by" value="firewall">

</form>

<?php $this->_include('panel/js/common'); ?>